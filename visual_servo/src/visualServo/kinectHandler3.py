...
class kinectHandler:
    ...
    def detect(self):
        #savePath = "/home/csl-mini-pc/Desktop/petropoulakis/experiment/"
        savePath = "/home/petropoulakis/Desktop/experiment/"
        frameCount = 0
        fpsText = ""
        type = 0
		...
		#print("\n")
            ...
            helpers.saveImage(colorFixed, savePath + "boxes/" + str(self.totalFrames) + ".jpg")


            if sum(score >= 0.5 for score in result["detection_scores"]) > 1 or result["detection_scores"][0] < 0.6:
                currPath = savePath + "more/" + str(self.totalFrames) + "_h.jpg"
                helpers.saveImage(currColor,currPath)

                if(self.totalFrames > 12):
                    break
            else:
                print("ok")
            # Plot #
            #self.pioneerDetector.drawBoxes(currColor, result)
            #fig = plt.figure()
            ...
            #cv2.putText(currColor, fpsText, org, font, fontScale, color, thickness, cv2.LINE_AA)
            #cv2.imshow("color", currColor)
